    ...
    for l_temp in linelist:
        ...
        if not go_on_deleting:
            ...
            if UNDERLINE in l_temp and SLASH in nl_temp:
                ...
                counter += 1
                if annotate:
                    returnlist.append(l_temp.replace(VERTLINE,EMPTYCHAR)
                                      +annotation+VERTLINE)
                else:
...
class Note_Shelf:
    ...
    def parse_sequence_key(self,
            ...
            """ takes the value of a sequence, following the identifier, and returns a tuple indicating
            the mark used for the sequence, the value itself, and the type of the value.
            """
...
##            self.display_buffer.append(alerts.NOTE_ADDED+BLANK+index_reduce(str(ind)))
                                    ...
                                    for x_temp in index.all_parents():
                                        if str(x_temp) in self.note_dict:
                                            keyset = keyset.union({y_temp for y_temp
                                                                   in self.note_dict[str(x_temp)].keyset
    ...
    def show(self,
        ...
        """
        if str(index) not in self.note_dict:
    ...
    def indexes(self):
        ...
        if not self.usesequence:
            ...
            if len(self.note_dict.keys()) != len(self.sortedindexes) \
        ...
        for k_temp in key_list:
            ...
            if nk_temp not in ['delete', 'Delete','d','D']:
                if nk_temp in self.keys() and nk_temp != EMPTYCHAR:
                    display.noteprint((alerts.ATTENTION,
                                       nk_temp+alerts.ALREADY_IN_USE+self.key_dict[nk_temp]))
...
##                           formkeys(entrylist)))
    ...
    def undelete(self,
        ...
        if undeletelist is None:
            undeletelist = [Index(x_temp) for x_temp in self.find_within(indexfrom=None,indexto=0)]
    ...
    def purge(self,
        ...
        for i_temp in [str(Index(n))
            ...
            if (len(str(self.note_dict[str(i_temp)].keyset)) < 5
                    and self.note_dict[str(i_temp)].text.replace(EOL,
    ...
    def enter(self,
        ...
        if not et and not em and not ek and (self.default_dict['enterhelp']\
                                             or self.default_dict['formattinghelp']):
...
##                                          if Index(a_temp) > Index(str(0))])
    ...
    def reconstitute_sequences(self,
        ...
        for entry in entrylist:
            if entry in self.note_dict:
                temp_keys = self.note_dict[entry].keyset
        ...
        for identifier in set(sequencedictionary['o'].keys()).union(set(sequencedictionary['d'].keys())).union(set(sequencedictionary['i'].keys())):
            ...
            if identifier in sequencedictionary['o']:
                ...
                if identifier not in sequencedictionary['d'] and identifier not in sequencedictionary['i']:
                    ...
                    for x_temp in sequencedictionary['o'][identifier]:
                        if x_temp.count(PERIOD)>1 or x_temp[-1]==PERIOD or not x_temp.replace(PERIOD,EMPTYCHAR).isnumeric():
                            all_float = False
                    if all_float:
                        seq_type = float
                    else:
                ...
                elif identifier in sequencedictionary['d'] and identifier not in sequencedictionary['i']:
                ...
                elif identifier in sequencedictionary['i'] and identifier not in sequencedictionary['d']:
                    nprint('Sequence '+identifier+' is inconsistent!')
        ...
        def recursive_show(dictionary, level=1):
            ...
            for k_temp in dictionary:
                ...
                if dictionary[k_temp] == {}:
                    ...
                    if self.default_dict['variablesize']:
                        if k_temp not in self.note_dict:
        ...
        for counter,index_temp in enumerate(index_list):
            if index_temp in self.indexes():
                showtext.append(str(counter+1)\
                                +' '+str(index_temp)+' : '\
                                +abridge(nformat\
                                         .format_keys(self.note_dict[str(index_temp)].keyset)))
...
##        print(flagvalue,flagvalue2)
        ...
        if failed:
            try:
                ...
                display.noteprint((alerts.ATTENTION,'CREATING NEW PICKLE DICTIONARY'))
...
##            self.default_dict['indexlist'] = OrderedList(self.indexes(),indexstrings=True)
##            
        if 'indexlist_indexes' not in self.default_dict:
    ...
    def determ_com (self,mainterm=EMPTYCHAR,
        ...
        if mainterm in ['changedeterminant','changedet']:
            ...
            self.default_dict['determinant'] = s_input(queries.DETERMINANT,
                                                       otherterms[0],
                                                       typeflag='str',
    ...
    def resize_etc_com (self,
        ...
        if mainterm in ['convertdefinitions']:
            for x_temp in ['d','s','e']:
                ...
                if inp_temp:
                    self.default_dict['convert'][self.default_dict['convertmode']].change(inp_temp,
                                                                                     x_temp)
                else:
                    self.default_dict['convert'][self.default_dict['convertmode']].change(defaults[x_temp],
                                                                                      x_temp)
        ...
        if mainterm in ['seqformone']:
            entry_temp = s_input(queries.SEQ_FORM_ONE,
                                 ...
                                 must_be_in=['s',
                                             ...
                                             'b',
                                             'n'],
                                 returnvalue=self.default_dict['seqform1'])
        ...
        if mainterm in ['invert']:
            ...
            self.default_dict['returnquit'] = int(s_input(queries.RETURN_QUIT,
                                                          ...
                                                          typeflag='int',
                                                          conditions=(2,10),
                                                          returnvalue=self.default_dict['returnquit']))
            self.dd_changed=True
            ...
            self.default_dict['size'] = int(s_input(queries.NEW_NOTE_SIZE,
                                                    ...
                                                    typeflag='int',
                                                    conditions=(10,300),
                                                    returnvalue=self.default_dict['size']))
            self.dd_changed=True
            ...
            self.default_dict['keytrim'] = int(s_input(queries.SET_KEY_TRIM,
                                                       otherterms[0],
                                                       typeflag='int',
                                                       conditions=(0,100),
                                                       returnvalue=self.default_dict['keytrim']))
            display.noteprint((labels.KEY_TRIM, str(self.default_dict['keytrim'])))
            self.dd_changed=True

        elif mainterm in ['texttrim']:
            self.default_dict['texttrim'] = int(s_input(queries.SET_TEXT_TRIM,
                                                        otherterms[0],
                                                        typeflag='int',
                                                        conditions=(0,200),
                                                        returnvalue=self.default_dict['texttrim']))
            display.noteprint((labels.TEXT_TRIM,
                               str(self.default_dict['texttrim'])))
            self.dd_changed=True
            ...
            if len(temp_range) > 10:
                ...
                for x_temp in temp_range:
                    ...
                    self.edit(Index(x_temp),
                              newkeyset=temp_keys,
                              newtext=self.note_dict[x_temp].text,
                              changekeys=False,
                              changetext=False,
                              update_table=False)
            ...
            if len(temp_range) > 1:
                ...
                for counter, x_temp in enumerate(temp_range):
                    ...
                    if counter == 0:
                        ...
                        self.edit(Index(x_temp),
                                  newkeyset=temp_keys,newtext=self.note_dict[x_temp].text,
                                  changekeys=False,
                                  changetext=False,
                                  update_table=False)
...
##                              param_is_emb=True)
            ...
            self.default_dict['user'] = s_input('User? ',
                                                otherterms[0],
                                                returnvalue=self.default_dict['user'])
            self.dd_changed=True
            ...
            temp_entry = s_input(queries.INDEX,
                                 otherterms[0],
                                 typeflag='index')
            if temp_entry in self.note_dict:
                print()
                print()
                print('****KEYSET****')
                print(str(self.note_dict[temp_entry].keyset))
                print()
                print('****TEXT****')
                print(self.note_dict[temp_entry].text.replace(EOL,'|'))
                print()
                print('****META****')
                print(str(self.note_dict[temp_entry].meta))
                print()
                print()
...
##                nprint('Iterator error #1')
        ...
        if self.indexchanges  == 1 or prefix == 'beta':
            self.counter = 0
    ...
    ##                    self.default_dict['defaultkeys'] += self.default_dict['projects'][project_name]['defaultkeys']
        ...
        ##                command_stack.add('skip:'+temp_uptohere)
            ...
            if 'archivedprojects' not in self.default_dict:
                self.default_dict['archivedprojects'] = {}
            if project_name not in self.default_dict['archivedprojects']:
                self.default_dict['archivedprojects'][project_name] = copy.deepcopy(self.default_dict['projects'][project_name])
                if self.default_dict['archivedprojects'][project_name] == self.default_dict['projects'][project_name]:
                    del self.default_dict['projects'][project_name]
                display.noteprint((alerts.ATTENTION,
            ...
            while True:
                ...
                if project_name and project_name in self.default_dict['archivedprojects']:
                    if input('ARE YOUR SURE?') in YESTERMS:
            ...
            if  'archivedprojects' in self.default_dict:
                ...
                while True:
                    ...
                    project_name = s_input('Project name?',otherterms[0])
                    if project_name in self.default_dict['archivedprojects']:
                        break
                if project_name not in self.default_dict['projects']:
                    self.default_dict['projects'][project_name] = copy.deepcopy(self.default_dict['archivedprojects'][project_name])
                    if self.default_dict['projects'][project_name] == self.default_dict['archivedprojects'][project_name]:
                        del self.default_dict['archivedprojects'][project_name]
            ...
            self.show_projects(projectobject=self.default_dict['archivedprojects'])
            ...
            if name_temp:
                ...
                text_temp += 'INDEXES: ' + ', '.join(transpose_keys(self.default_dict['projects'][name_temp]['indexes'].list,
                                                                    surround=False))
...
##        try:
            ...
            if inputterm not in list(NOTERMS) + list(QUITTERMS):
                if command_stack.size() > 0:
