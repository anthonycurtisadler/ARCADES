...
##                    self.key_dict[j_temp].add(str(Index(i_temp)))
    ...
    def kindler (self):
        ...
        import kindle
        from numbertools import rom_to_int
        ...
        try:
            note_text = get_text_file(input('FILENAME?'))
        except:
        ...
        include_page = input('Include sequence key for pages?') in YESTERMS_PLUS
        include_part = input('Include sequence key for part?') in YESTERMS_PLUS
        include_location = input('Include sequence key for location?') in YESTERMS_PLUS
        ...
        while True:
            ...
            if count_down > 0:
                ...
                highlight_color, part, page, location, text, note_part = note
                ...
                if temp_i in QUITTERMS:
                    ...
                    if include_page and page:
                        new_keys.add('page@'+page)
                    if include_location and location:
                        new_keys.add('location@'+location)
                    if include_part and part:
                        new_keys.add('part@'+str(rom_to_int(part.lower())))
